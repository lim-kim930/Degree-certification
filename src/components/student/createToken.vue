<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>创建令牌</el-breadcrumb-item>
    </el-breadcrumb>
  ...
  methods: {
    ...
    submitForm() {
      ...
      for (var i = 0; i < this.ruleForm.type.length; i++) {
        params.range += this.rangeFormat(this.ruleForm.type[i]) + ",";
      }
      ...
      this.axios
        ...
        .then(function (response) {
          ...
          that
            .axios({
              method: "post",
              url: "https://api-vnt.fdkevin.xyz/generateToken",
              data: params,
              headers: { Authorization: keystore, code: code },
            })
            .then(function (response) {
              params = { range: "" };
              that.openSuccess(response.data.tokenKey, response.data.txHash);
              that.loading = false;
    ...
    openSuccess(token, hash) {
      this.$confirm(
        "令牌值: " + token + "<i class='el-icon-document-copy el-icon--right' onmouseover="+"this.style.color='#409eff'"+" onmouseout="+"this.style.color='#606266'"+" onclick='copyToken()' style='cursor: pointer;' title='复制令牌'></i><br>" + "此次交易号: " + hash + "</a>",
        "令牌生成成功!",
        {
